...
async function readPackageJson(): Promise<PackageJson> {
  ...
  return {
    ...
    ...(isObject(scripts) && {
      scripts: {
        ...(typeof scripts["crawl"] === "string" && { crawl: scripts["crawl"] }),
      },
    }),
  }
}
...
describe("US-001: Project Structure", () => {
  ...
  it("should have tsconfig.node.json for type checking", async () => {
    await access("tsconfig.node.json", constants.F_OK)
  })
})
